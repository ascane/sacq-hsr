...
class QNetwork(generic.QNetwork):
    ...
    def get_q_delta(self, trajectory, task):
        ...
        # Calculate all values of pi(* | state, task_id) for the trajectory for out current task
        #   (using the fixed network)
        policies = self.p_network.distribution_array(states, task, live=False)


        # Pick all pi(a_t | state, task_id) from pi(* | state, task_id) for every action taken in the trajectory
