...
class PolicyNetwork:
    ...
    def distribution(self, state, task):
        s = self.state_transformer(state)
        s = np.expand_dims(s, axis=0)
        return self.model.predict(s, task)[0]

    def distribution_array(self, states, task, live=True):
    ...
    def sample(self, state, task, dist=None):
        if dist is None:
    ...
    def sample_greedy(self, state, task):
        raise NotImplementedError

    def sample_epsilon_greedy(self, state, task):
        raise NotImplementedError

    def sample_distribution(self, state, task):
        dist = self.distribution(state, task)
...
if __name__ == "__main__":
    import sacx.mock_tasked_q_network as mock
    def shared_net(x):
