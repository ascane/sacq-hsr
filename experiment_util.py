...
from inspect import getcallargs, getargspec, getargvalues, currentframe
import collections
...
def configurable(init):
    ...
    def dict_from_fn(fn):
        ...
        init_args, _, _, init_defaults = getargspec(fn)
        ...
        return merge_two_dicts(defaults, others)

    def dict_from_frame(frame, fn):
        args, vargs, kwargs, locals = getargvalues(frame)
        fn_args, _, _, fn_defaults = getargspec(fn)
        ...
        return merge_two_dicts(merge_two_dicts(args_dict, vargs_dict), kwargs_dict)
...
class Configurable():
    ...
    def get_configuration(self, c=None):
        def solve(k,v):
            if k is "self":
        ...
        if c is None: #base case
            ...
            return self.get_configuration(merge_two_dicts(self.__defaultConfiguration__, self.__configuration__))
